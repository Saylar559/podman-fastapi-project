.PHONY: up down restart logs purge ensure-env wait-for-backend wait-for-frontend open-browser show-summary kill-port restart-frontend logs-frontend push status log pull
...
wait-for-backend:
	...
	for i in $$(seq 1 60); do \
		CODE=$$(curl -s -o /dev/null -w "%{http_code}" "$(BACKEND_HEALTHCHECK_URL)" || echo 000); \
		if [ "$$CODE" = "200" ]; then \
			echo "✅ Бэкенд отвечает (HTTP $$CODE)"; exit 0; \
		fi; \
		sleep 1; \
	done; \
	...
	@if [ -n "$(FRONTEND_SERVICE)" ]; then \
		...
		for i in $$(seq 1 90); do \
			...
			if [ "$$CODE" = "200" ] || [ "$$CODE" = "301" ] || [ "$$CODE" = "302" ]; then \
				echo "✅ Фронтенд отвечает (HTTP $$CODE)"; exit 0; \
			fi; \
			sleep 1; \
		done; \
