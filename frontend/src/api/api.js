...
// ✅ Интерцептор ответов: обработка 401 (опционально можно дергать refresh)
api.interceptors.response.use(
  (response) => response,
  async (error) => {
    if (error.response?.status === 401) {
      console.warn("⚠️ 401 Unauthorized — возможно, токен протух");
      // Здесь можно вызвать refresh() и повторить запрос
      // Но пока просто пробрасываем ошибку
    }
...
/**
 * Скачивание отчёта (Excel в виде Blob)
 */
